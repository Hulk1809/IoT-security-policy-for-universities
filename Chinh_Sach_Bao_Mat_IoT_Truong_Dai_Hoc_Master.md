...
### 2.3. Mô Hình STRIDE Cho Hệ Thống IoT Trường Đại Học
...
### 2.4. Ánh Xạ Chi Tiết Lên OWASP IoT Top 10
...
### 2.5. Phân Tích và Tính Toán Điểm Số Lỗ Hổng CVSS v3.1 Chi Tiết
...
### 2.6. Mô hình ma trận RACI trong quản trị an ninh mạng Đại học
...
| **SW-03** | Phần mềm | Firmware Camera & Thiết bị đầu cuối | Thiết bị đầu cuối | Các phân vùng VLAN | **Trung bình (Medium)**| Mã nguồn chạy trực tiếp trên chip điều khiển phần cứng. |
...
| **DT-03** | Dữ liệu | Thông tin cấu hình & Token xác thực | Database Server | VLAN 99 (Quản trị) | **Chí mạng (Critical)**| Mật khẩu thiết bị, API Key, Token JWT kết nối Cloud. |
...
### 3.2. Khung Đánh Giá Rủi Ro Định Lượng (Risk Assessment Framework)
...
*   **Khả năng xảy ra (L)**: 
    *   `1 (Thấp)`: Khó xảy ra, yêu cầu kỹ thuật tấn công phức tạp.
    *   `2 (Trung bình)`: Có thể xảy ra trong điều kiện mạng nội bộ mở.
    *   `3 (Cao)`: Rất dễ xảy ra bằng các kịch bản quét tự động nếu giữ cấu hình mặc định.
*   **Mức độ tác động (I)**: 
    *   `1 (Thấp)`: Ảnh hưởng cục bộ, không mất mát dữ liệu.
    *   `2 (Trung bình)`: Ảnh hưởng một phân vùng mạng, rò rỉ dữ liệu thường.
    *   `3 (Cao)`: Ảnh hưởng toàn mạng, hỏng hóc thiết bị vật lý, rò rỉ dữ liệu sinh trắc học chí mạng.

#### **Ma trận xếp hạng rủi ro (Risk Matrix):**
*   🔴 **Điểm 7 - 9**: **Rủi ro Cao (High)** $\rightarrow$ Bắt buộc phải ưu tiên xử lý ngay lập tức trong 48h để bảo vệ hệ thống.
*   🟡 **Điểm 4 - 6**: **Rủi ro Trung bình (Medium)** $\rightarrow$ Cần lập kế hoạch khắc phục trong 1-2 tuần.
*   🟢 **Điểm 1 - 3**: **Rủi ro Thấp (Low)** $\rightarrow$ Chấp nhận rủi ro hoặc theo dõi định kỳ hàng tháng.

---

### 3.3. Bảng Đánh Giá Rủi Ro Tài Sản IoT Chi Tiết
...
### 3.4. Xác định các chủ thể và vai trò trong hệ thống
...
### 3.5. Tiêu chí đánh giá bảo mật dựa trên chuẩn OWASP IoT
...
### 4.2. Các Chính Sách Quản Lý IoT Bổ Sung Trong Trường Đại Học
...
- **Đầu vào (Procurement)**: Mọi thiết bị IoT trước khi mua sắm và lắp đặt phải qua sự phê duyệt của phòng IT về mặt thông số an toàn (có hỗ trợ thay đổi mật khẩu, mã hóa truyền thông, cập nhật firmware).
- **Vận hành (Operations)**: Thiết lập lịch quét IP định kỳ hàng tháng để phát hiện các thiết bị IoT không rõ nguồn gốc (Shadow IoT) kết nối vào mạng trường.
- **Đầu ra (Decommissioning)**: Trước khi thanh lý thiết bị IoT cũ, phải thực hiện khôi phục cài đặt gốc (Factory Reset) và xóa sạch các khóa mật mã, dữ liệu cấu hình cũ bên trong chip nhớ.
...
### 4.3. Ma trận Phân quyền RACI Chi tiết
...
### 4.4. Quy trình Vận hành Bảo mật (Workflow Diagrams)

#### 4.4.1. Quy trình Lắp đặt và Hardening Thiết bị IoT Mới
```mermaid
graph TD
...
#### 4.4.2. Quy trình Phản ứng và Cô lập Sự cố Khẩn cấp
```mermaid
graph TD
...
- [ ] **Mã hóa truyền thông**: Thiết bị có hỗ trợ các giao thức mã hóa an toàn không? (HTTPS thay vì HTTP, MQTTS thay vì MQTT, RTSPS/SRTP thay vì RTSP).
...
### 5.4. Minh Chứng & Kịch Bản Kiểm Thử Bảo Mật Trực Quan (Penetration Testing Scenarios)

Để chứng minh chính sách bảo mật hoạt động hiệu quả, chúng tôi thực hiện 3 kịch bản kiểm thử trực quan trên giao diện **Web Dashboard**:
...
*   **Minh chứng kết quả (Evidence)**:
    *   Trạng thái mạng của Camera **HW-01** đổi từ `Trực tuyến` sang `CÔ LẬP MẠNG` (màu xám tắt kết nối trong 2.4s).
...
### 6.2. Kế Hoạch Ưu Tiên Xử Lý & Sắp Xếp Lộ Trình (Treatment & Prioritization Plan)
...
1.  **Ưu tiên 1 (Khẩn cấp - Trong 48h)**: 
    *   Tiến hành đổi mật khẩu mặc định của toàn bộ hệ thống Camera IP (RISK-01 / HW-01).
    *   Cấu hình phân vùng VLAN 30 an ninh cách ly và vô hiệu hóa các cổng Telnet (23), FTP (21).
2.  **Ưu tiên 2 (Ngắn hạn - Trong 1-2 tuần)**: 
    *   Bật mã hóa SSL/TLS cho máy điểm danh RFID & Smart Lock (RISK-02 / HW-02).
    *   Nâng cấp thẻ từ của cán bộ giảng viên sang loại mã hóa Mifare DESFire EV2/EV3 chống sao chép.
3.  **Ưu tiên 3 (Trung hạn - Trong 1 tháng)**: 
    *   Cấu hình Whitelist IP ACL cho bộ điều khiển HVAC (RISK-03 / HW-05) và phân vùng VLAN 20 cho máy chiếu giảng đường (RISK-04 / HW-04).
    *   Chạy chương trình cập nhật bản vá lỗ hổng OS nhúng cho các thiết bị IoT Gateway (HW-03).
...
3.  Thiết lập các sơ đồ Quy trình Vận hành chuẩn, Bộ Cẩm nang Checklist kiểm tra định kỳ 4 giai đoạn và xây dựng thành công ứng dụng Web Dashboard mô phỏng an ninh trực quan.
